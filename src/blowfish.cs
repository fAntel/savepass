	...
	public class blowfish
	{
		...
		public byte[] decrypt(byte[] text)
		{
			...
			for (int i = 0; i < len; i += 8) {
				...
				Buffer.BlockCopy(block, 0, prev_block, 0, 8);
				decrypt_block(ref block);
		...
		private void decrypt_block(ref byte[] block)
		{
			uint xl, xr;
		...
		private void encrypt_block(ref byte[] block)
		{
			uint xl, xr;
			split_block(ref block, out xl, out xr);
		...
		private void concate_block(ref byte[] block, uint xl, uint xr)
		{
			byte[] xl_block = new byte[4];
			byte[] xr_block = new byte[4];
			xl_block = BitConverter.GetBytes(xl);
			xr_block = BitConverter.GetBytes(xr);
			if (!BitConverter.IsLittleEndian) {
				Array.Reverse(xl_block);
				Array.Reverse(xr_block);
			}
			Buffer.BlockCopy(xl_block, 0, block, 0, 4);
			Buffer.BlockCopy(xr_block, 0, block, 4, 4);
		}
		...
		private uint F(uint x)
		{
			...
			x >>= 8;
			a = x & 0x00FF;
			// y = ((S[0, x[0]] + S[1, x[1]]) ^ S[2, x[2]]) + S[3, x[3]]
			y = ((S[0, a] + S[1, b]) ^ S[2, c]) + S[3, d];

			return y;
		}

		private void split_block(ref byte[] block, out uint xl, out uint xr)
		{
			byte[] xl_block = new byte[4];
			byte[] xr_block = new byte[4];
			Buffer.BlockCopy(block, 0, xl_block, 0, 4);
			Buffer.BlockCopy(block, 4, xr_block, 0, 4);
			if (!BitConverter.IsLittleEndian) {
				Array.Reverse(xl_block);
				Array.Reverse(xr_block);
			}
			xl = BitConverter.ToUInt32(xl_block, 0);
